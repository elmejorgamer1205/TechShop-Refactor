...
public class GestorPedidos {
     ...
     *         "Envío Estándar" para menos de 5 productos (inclusive),
     *         "Envío Descuento" para entre 6 y 9 productos,
     *         "Envío Premium" para 10 o más productos
     */
    public String evaluarEnvio(int numeroProductos) {
        if (numeroProductos <= 0) {
            throw new IllegalArgumentException("El número de productos debe ser mayor a 0");
        }

        if (numeroProductos <= LIMITE_ENVIO_ESTANDAR) {
            return "Envío Estándar";
        } else if (numeroProductos < LIMITE_ENVIO_PREMIUM) {
            return "Envío Descuento";
        } else {
